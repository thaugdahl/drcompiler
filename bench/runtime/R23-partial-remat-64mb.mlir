...
module {
  func.func @main() -> i32 {
    ...
    %result = affine.for %rep = 0 to 5 iter_args(%outer = %cst) -> (f64) {
      affine.for %i = 0 to 8388608 {
        ...
        affine.store %val, %buf[%i] : memref<8388608xf64>
      }
      %inner = affine.for %j = 0 to 8388608 iter_args(%acc = %cst) -> (f64) {
        %v = affine.load %buf[%j] : memref<8388608xf64>
