...
// B018: Partial remat — chain with 2 non-chainable leaf loads.

module {
  func.func @partial_two_leaves() -> f32 {
    %c0 = arith.constant 0 : index
    // expected-remark @+1 {{cost-model:}}
    %src1 = memref.alloc() : memref<4xf32>
    // expected-remark @+1 {{cost-model:}}
    %src2 = memref.alloc() : memref<4xf32>
    ...
    affine.for %i = 0 to 4 {
      ...
      affine.store %f, %src1[%i] : memref<4xf32>
      affine.store %f, %src2[%i] : memref<4xf32>
    }

    affine.for %j = 0 to 1048576 {
      // expected-remark @below {{full-remat: REJECT_UNSAFE}}
      // expected-remark @below {{load: SINGLE}}
      %v1 = affine.load %src1[0] : memref<4xf32>
      // expected-remark @below {{full-remat: REJECT_UNSAFE}}
      // expected-remark @below {{load: SINGLE}}
      %v2 = affine.load %src2[0] : memref<4xf32>
      %s = arith.addf %v1, %v2 : f32
    ...
    memref.dealloc %src1 : memref<4xf32>
    memref.dealloc %src2 : memref<4xf32>
    memref.dealloc %dst : memref<1048576xf32>
    return %out : f32
  }
}

// CHECK-LABEL: func.func @partial_two_leaves
// CHECK:         affine.for
// CHECK:         affine.for
// CHECK:         }
