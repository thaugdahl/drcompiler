...
//
// D009: affine loop trip=8 (buffer 8*8=64B, fits L1).
// Chain: mulf(3) + addf(1) + sqrt(20) = 24.
// recompute = 2*24 = 48, keep = 24 + 1 + 2*4 = 33 → FISSION.

module {
  func.func @run(%x: memref<8xf64>) -> f64 {
    ...
    %sum = affine.for %i = 0 to 8 iter_args(%acc = %cst0) -> (f64) {
      %xi = affine.load %x[%i] : memref<8xf64>
    ...
    %max = affine.for %i = 0 to 8 iter_args(%best = %cst0) -> (f64) {
      %xi = affine.load %x[%i] : memref<8xf64>
      %sq = arith.mulf %xi, %xi : f64
      %s  = arith.addf %sq, %cst1 : f64
      %r  = math.sqrt %s : f64
