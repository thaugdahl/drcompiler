// RUN: dr-opt %s --pass-pipeline='builtin.module(memory-fission{test-diagnostics l1-latency=4})' -verify-diagnostics | FileCheck %s
//
// D017: small trip, buffer fits L1 (lat=4), moderate chain cost.
// Chain: mulf(3) + addf(1) + sqrt(20) = 24.  consumers=2.
// keep = 24 + 1 + 2*4 = 33.  recompute = 2*24 = 48.
// 33 < 48 → FISSION.

module {
  func.func @run(%x: memref<64xf64>) -> f64 {
    ...
    %sum = affine.for %i = 0 to 64 iter_args(%acc = %cst0) -> (f64) {
      %xi = affine.load %x[%i] : memref<64xf64>
    ...
    %max = affine.for %i = 0 to 64 iter_args(%best = %cst0) -> (f64) {
      %xi = affine.load %x[%i] : memref<64xf64>
      %sq = arith.mulf %xi, %xi : f64
      %s  = arith.addf %sq, %cst1 : f64
      %r  = math.sqrt %s : f64
