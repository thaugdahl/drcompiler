...
for N in 1024 2048; do
  ...
  $DROPT --allow-unregistered-dialect --pass-pipeline='builtin.module(func.func(dr-affine-loop-tile))' "$D/g.mlir" -o "$D/gt.mlir" 2>/dev/null
  ...
  $DROPT --allow-unregistered-dialect --pass-pipeline='builtin.module(func.func(dr-affine-loop-tile),dr-par-bubbles{par-spmd-perband},func.func(convert-par-to-omp))' "$D/g.mlir" -o "$D/pt.mlir" 2>"$D/e2"
