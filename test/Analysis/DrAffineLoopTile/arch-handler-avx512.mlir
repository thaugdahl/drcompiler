...
// arch-handler + spill-strategy options still parse (kept for pipeline
// compatibility; the v2 reuse model does not consume them).  The spatial-only
// stencil carries no per-reference temporal reuse, so v2 REJECTS and the
// nest must come through untouched — v1 tiled it for pure overhead.
